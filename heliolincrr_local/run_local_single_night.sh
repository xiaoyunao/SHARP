...
ROOT_DATA="${ROOT_DATA:-${PROJECT_ROOT}/local_data}"
CODE_DIR="${PROJECT_ROOT}/heliolincrr_local"

# 默认 tracklets ALL 路径（你按之前流程一般就是这个）
DEFAULT_ALL="${ROOT_DATA}/heliolincrr/${NIGHT}/tracklets_linreproj/tracklets_${NIGHT}_ALL.fits"
TRACKLETS_ALL="${2:-$DEFAULT_ALL}"

RR_CORES=4

# RR 输出目录
RR_NIGHT_DIR="${ROOT_DATA}/heliolincrr/${NIGHT}/rr_links"
