...
- Figure 06 is a lossless one-row extraction of the three frames in the author-specified `20260210_00000P2_link0035.gif`. No new labels, arrows, registration, or other overlays were added.
...
- `figures/`: 12 vector PDF masters and matching 300-dpi PNG previews. Figure 05c was removed at the author's request.
...
Run `scripts/run_all.sh` from the repository root after providing the exact ASTORB catalog, the single Figure 01 calibrated L1 file, and the author-specified Figure 06 GIF listed in the script. The general analysis uses `/opt/anaconda3/bin/python3`; representative short-arc track recomputation uses Python 3.10 with `poliastro==0.17.0`. Raw calibrated images, the source GIF, and the 407-MB ASTORB catalog are intentionally not included in the ZIP; their identities and checksums are preserved in the audit.

The plotting implementation is source-anchored rather than visually approximated. `GOTTA_SOURCE_MAP.md` identifies the exact GOTTA source file and plotting grammar used for every figure family, while `scripts/gotta_reference_source/` contains checksum-preserved copies of those source files.
