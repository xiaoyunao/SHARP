...
Other constraints to retain: known-sky products are detection-weighted; orbit/revisit products are unique-object weighted; candidate diagnostics are linkage-level; trajectory/registration products are measurement-level. The 1 arcsec line is a nominal pipeline threshold, not a completeness/contamination result. PHA is unavailable from the frozen ASTORB source; NEO is transparently derived as `q < 1.3 AU`.
