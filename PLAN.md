...
- `summarize_single_night.py` 能稳定写出 summary 和 unknown catalog
- `plot_unknown_links.py` 能稳定按 unknown catalog 产出 GIF 和 summary
- 正式单夜目录中不保留旧试验目录与旧分析副本

## Next recommended steps

1. 保持当前单夜正式参数 `speed=5`, `direction=10`, `require_shared_endpoint=True`, `orbit max_v=35` 不变
2. 若后续继续提升单夜召回，只围绕线性 linking 的边规则做小范围扫描
3. 单夜 orbit confirm 继续作为后验验证，不回到旧 RR 单夜聚类路线
